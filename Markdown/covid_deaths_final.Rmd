...
### Republican Votes in the 2020 Presidential Elections
```{r govMap}
# Saving a static version of the figure (capitalizing on the tmap package)
...
The following dates were removed from our data frame since the `scaledNEWMA7` variable was constant across all counties: `r print(datesDropped, sep =", ")`.
...
- Using a multinomial regression model, we have shown/quantified the impact of the following factors: `r select(multiClassDF, -c(key_local, cluster_group, location)) %>% colnames() %>% pander()` on the odds of being in a specific cluster when compared to the baseline.
