...
default_sizing = function(){r2d3::sizingPolicy(padding = 0, browser.padding = 0, browser.fill = TRUE)}
...
pacman::p_load(tidyverse, magrittr, janitor, dataPreparation, lubridate, skimr, # for data analysis
               ...
               DT, pander, stargazer, # for formatting and nicely printed outputs
               scales, RColorBrewer, DataExplorer, # for plots
               plotly, albersusa, r2d3maps, tigris, leaflet, # for maps
...
# Saving cumulative deaths figure to a png file
png(filename = '../Figures/sampleCumulativeCases.png',
...
# Saving cumulative deaths figure to a png file
png(filename = '../Figures/sampleCumulativeDeaths.png',
    width = 1366, height =768, pointsize = 16)
counties %>% filter(id %in% idIndex) %>% 
...
### RPL Theme 1
```{r rpl1Map, fig.height=1, results='asis'}
...
# Creating an interactive d3 map
d3map <- d3_map(shape = arrange(cty_sf, region), projection = "Albers") %>%
  add_continuous_breaks(var = "rpl_theme1", palette = "Greens", direction = -1) %>%
  add_tooltip(value = paste("<b>{name}</b> : RPL Theme 1", "{rpl_theme1}")) %>% 
  add_legend(title = "RPL Theme 1: Percentile Ranking of Counties") %>%
  add_labs(caption = "Data: CDC’s Social Vulnerability Index Database 2018 U.S.")
  
# Saving a static version of the figure
r2d3::save_d3_png(d3map, file = '../Figures/rplTheme1Map.png', 
                  width = 1366, height =768, zoom = 1, delay = 3) 

d3map # printing it out
```

### RPL Theme 2
```{r rpl2Map, fig.height=1, results='asis'}
# Creating an interactive d3 map
d3map <- d3_map(shape = arrange(cty_sf, region), projection = "Albers") %>%
  add_continuous_breaks(var = "rpl_theme2", palette = "Greens", direction = -1) %>%
  add_tooltip(value = paste("<b>{name}</b> : RPL Theme 2", "{rpl_theme2}")) %>% 
  add_legend(title = "RPL Theme 2: Percentile Ranking of Counties") %>%
  add_labs(caption = "Data: CDC’s Social Vulnerability Index Database 2018 U.S.")
  
# Saving a static version of the figure
r2d3::save_d3_png(d3map, file = '../Figures/rplTheme2Map.png', 
                  width = 1366, height =768, zoom = 1, delay = 3) 
...
### RPL Theme 3
```{r rpl3Map, fig.height=1, results='asis'}
# Creating an interactive d3 map
d3map <- d3_map(shape = arrange(cty_sf, region), projection = "Albers") %>%
  add_continuous_breaks(var = "rpl_theme3", palette = "Greens", direction = -1) %>%
  add_tooltip(value = paste("<b>{name}</b> : RPL Theme 3", "{rpl_theme3}")) %>% 
  add_legend(title = "RPL Theme 3: Percentile Ranking of Counties") %>%
  add_labs(caption = "Data: CDC’s Social Vulnerability Index Database 2018 U.S.")
  
# Saving a static version of the figure
r2d3::save_d3_png(d3map, file = '../Figures/rplTheme3Map.png', 
                  width = 1366, height =768, zoom = 1, delay = 3) 
...
### RPL Theme 4
```{r rpl4Map, fig.height=1, results='asis'}
# Creating an interactive d3 map
d3map <- d3_map(shape = arrange(cty_sf, region), projection = "Albers") %>%
  add_continuous_breaks(var = "rpl_theme4", palette = "Greens", direction = -1) %>%
  add_tooltip(value = paste("<b>{name}</b> : RPL Theme 4", "{rpl_theme4}")) %>% 
  add_legend(title = "RPL Theme 4: Percentile Ranking of Counties") %>%
  add_labs(caption = "Data: CDC’s Social Vulnerability Index Database 2018 U.S.")
  
# Saving a static version of the figure
r2d3::save_d3_png(d3map, file = '../Figures/rplTheme4Map.png', 
                  width = 1366, height =768, zoom = 1, delay = 3) 
...
```{r percRepVotesMap, fig.height=1, results='asis'}
# Creating an interactive d3 map
d3map <- d3_map(shape = arrange(cty_sf, region), projection = "Albers") %>%
  add_continuous_gradient2(var = "percRepVotes", low = '#2166AC', mid = '#F7F7F7', high = '#B2182B',
                           range = c(0, 50, 100)) %>%
  add_tooltip(value = paste("<b>{name}</b> : Percent Republican Votes in 2016 Presidential Elections", "{percRepVotes}")) %>% 
  add_legend(title = "Percent Republican Votes in 2016 Presidential Election", suffix = "%") %>%
  add_labs(caption = "Data: Election Data Archive Dataverse (Harvard University)")
  
# Saving a static version of the figure
r2d3::save_d3_png(d3map, file = '../Figures/percRepVotesMap.png', 
                  width = 1366, height =768, zoom = 1, delay = 3) 
...
### CDC Regions
```{r cdcMap, fig.height=1, results='asis'}
...
# Creating an interactive d3 map
d3map <- d3_map(shape = arrange(state_sf, region), projection = "Albers") %>%
  add_discrete_scale(var = "region", palette = "Paired") %>%
  add_tooltip(value = paste("<b>{name}</b> : CDC Region", "{region}")) %>% 
  add_legend(title = "Region") %>%
  add_labs(caption = "Data: CDC’s National Center for Chronic Disease Prevention and Health Promotion")
  
# Saving a static version of the figure
r2d3::save_d3_png(d3map, file = '../Figures/cdcMap.png', 
                  width = 1366, height =768, zoom = 1, delay = 3) 
...
```{r govMap, fig.height=1, results='asis'}
# Creating an interactive d3 map
d3map <- d3_map(shape = arrange(state_sf, region), projection = "Albers") %>%
  add_discrete_scale2(var = "party", values = list('Democratic' = '#1F78B4',
                                                   'Republican' = '#E31A1C')) %>%
  add_tooltip(value = paste("<b>{name}</b> \n Governor's Party:", "{party}")) %>% 
  add_legend(title = "Gov. Party") %>%
  add_labs(caption = paste0("Data: Wikipedia (as of ", endDatePrintV, ")"))
  
# Saving a static version of the figure
r2d3::save_d3_png(d3map, file = '../Figures/governorsMap.png', 
                  width = 1366, height =768, zoom = 1, delay = 3) 
...
# Saving spaghetti plot to a png file
png(filename = '../Figures/spaghettiPlot.png', width = 1366, height =768, pointsize = 16)
spaghettiDF %>%  
...
# Creating an interactive plot for the markdown
p <- ggplot2::last_plot() + geom_line(size = 0.25, alpha = 0.1) + # modifying the plot for plotly
  theme_bw(base_size = 9) + theme(legend.position = 'none') # to make margins smaller
ggplotly(p, height = 768) %>%  layout_ggplotly()
```
...
png(filename = '../Figures/summaryPlot.png', width = 1366, height =768, pointsize = 16)
summaryDf %>% 
...
# Creating an interactive plot for the markdown
p <- ggplot2::last_plot() + # modifying the plot for plotly
  theme_bw(base_size = 9)  # to make margins smaller
ggplotly(p, height = 768) %>% layout_ggplotly()
```
...
d3map <- d3_map(shape = arrange(cty_sf, cluster_group), projection = "Albers") %>%
  add_discrete_scale2(var = "cluster_group", values = colorPal) %>%
  add_tooltip(value = paste("<b>{name}</b> : Cluster #", "{cluster_group}")) %>% 
  add_legend(title = "Cluster")

r2d3::save_d3_png(d3map, file = '../Figures/clusterMap.png', 
                  width = 1366, height =768, zoom = 1, delay = 3) 
...
```{r corrPlot}
png(filename = '../Figures/corrPlot.png',
    width = 1366, height =768, pointsize = 16)
...
```{r boxPlot}
# Saving cumulative deaths figure to a png file
png(filename = '../Figures/boxPlot.png',
    width = 1366, height =768, pointsize = 16)
multiClassDF %>% 
...
stargazer(finalModel, type = 'html', p.auto = FALSE, out="../Data/Output/multi.html", single.row = TRUE)
```
...
# Creating a static visual for the paper
d3map <- d3_map(shape = arrange(cty_sf, match), projection = "Albers") %>%
  add_discrete_scale2(var = "match", 
                      values = list("Yes" = "#CAB2D6", "No" = "#6A3D9A"),
                      na_color = 'white') %>%
  add_tooltip(value = paste("<b>{name}</b> : Match", "{match}")) %>% 
  add_legend(title = "Match")

r2d3::save_d3_png(d3map, file = '../Figures/clusterMatchMap.png', 
                  width = 1366, height =768, zoom = 1, delay = 3) 
...
## Removing White Space in Static Maps
In this appendix, we have written a short script to remove the automatically generated white space from the `save_d3_png()` function. To do this, we have capitalized on the [magick package](https://cran.r-project.org/web/packages/magick/vignettes/intro.html). Obviously, the reader does not need to run this code; however, we are adding it here for the sake of completion given that the trimmed figures are what we are planning to add to our submitted manuscripts.

```{r trimImageMargins}
# This chunk of code is written, but not included in the HTML output
# Its sole purpose is to trim white space in all the generated d3 static maps
pacman::p_load(magick)
imageFiles <- list.files(path='../Figures/', pattern='Map.png', full.names = TRUE)
for (i in 1:length(imageFiles)) {
image_read(imageFiles[i]) %>% # reading all images with the suffix Map.png
    image_trim() %>% # capitalizing on the auto-cropping functionality from the magick package
    image_write(path = paste0(str_sub(imageFiles[i], start=1, end=-5), '_trimmed.png'), 
                format = 'png')  # save trimmed image to png in the same directory with _trimmed in name
}
```